...
def ros_spin_thread(node):
    ...
    try:
        ...
        node.destroy_node()
        if rclpy.ok():  # ✅ Verifica che ROS sia ancora attivo prima di spegnerlo
            rclpy.shutdown()
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data["type"] == "offer":
                ...
                while True:
                    odom = (f"📍 Posizione: x={format(node.last_position[0], '.2f')}, "
                            f"y={format(node.last_position[1], '.2f')} | 🔄 Yaw = "
                            f"{format(math.degrees(node.last_orientation[2]), '.2f')}° | "
                            f"⌨️ Comando: {node.last_command}")
                    print(odom)
        ...
        if peer_connection:
            await peer_connection.close()  # ✅ Chiude correttamente il peer

        node.destroy_node()  # ✅ Distruggiamo il nodo ROS 2
